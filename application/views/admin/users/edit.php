    ...
    <div class="card mb-3">
      ...
      <div class="card-body">
        <div class="row justify-content-center">
			<form class="col-md-6 form" msg="Updating User Data" action="<?php echo site_url('admin/user/update') ?>" method="POST">
				
				
				<div class='form-group'>
					<label>Full Name</label>
					<input type="text" name="full_name" class="form-control" placeholder="Full Name" required value="<?php echo $userData->full_name; ?>"/>
					<input type="hidden" name="user_id" value="<?php echo $userData->user_id; ?>"/>
				</div>

				<div class='form-group'>
					<label>Email</label>
					<input type="email" name="email" class="form-control" placeholder="Email" required value="<?php echo $userData->email; ?>"/>
				</div>
					...
					<?php $cooperatives = $this->db->get_where('cooperatives', ['active' => 1])->result(); ?>
          			<select class="form-control" name="cooperative_id">
						<?php foreach ($cooperatives as $Key => $cooperative) { ?>
							<option 
								<?php echo ($userData->cooperative_id == $cooperative->cooperative_id) ? ' selected ': ''; ?>
